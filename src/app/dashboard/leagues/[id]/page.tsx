        ...
        try {
            ...
            if (memberIds.length > 0) {
                ...
                if (portfolioIds.length > 0) {
                    // Delete operation - relying on CASCADE constraints would be best but manual for now
                    // 1. Delete Holdings
                    const { error: holdingsErr } = await supabase.from('holdings').delete().in('portfolio_id', portfolioIds);
                    if (holdingsErr) console.error("Holdings delete error:", holdingsErr);

                    // 2. Delete Portfolios
                    const { error: portsErr } = await supabase.from('portfolios').delete().in('id', portfolioIds);
                    if (portsErr) console.error("Portfolios delete error:", portsErr);

                    // 3. Delete Members
                    const { error: membersErr } = await supabase.from('league_members').delete().eq('league_id', league.id);
                    if (membersErr) console.error("Members delete error:", membersErr);

                    // 4. Delete League
                    const { error: leagueErr } = await supabase.from('leagues').delete().eq('id', league.id);
                    if (leagueErr) {
                        console.error("League delete error:", leagueErr);
                        throw new Error(leagueErr.message);
                    }
